...
import { gsap, Elastic, Expo, Bounce } from "gsap";
import "../DrumKit.css"

const DrumKit = ({classe}) => {
  const hiHati = useRef(0);

  useEffect(() => {
    const get = (id) => document.getElementById(id);

    const crashCymbolAll = get("Crash");
    const crashCymbol = get("Crash-Cymbol");
    const crashAudio = get("Crash-Audio");

    const rightTomDrumAll = get("Tom-Right-All");
    const rightTomDrum = get("Tom-Right-Drum");
    const smallTomAudio = get("Small-Rack-Tom-Audio");

    const leftTomDrumAll = get("Tom-Left-All");
    const leftTomDrum = get("Tom-Left-Drum");
    const bigTomAudio = get("Big-Rack-Tom-Audio");

    const hiHatAll = get("Hi-Hat");
    const hiHatTop = get("Hi-Hat-Top");
    const hiHatBottom = get("Hi-Hat-Bottom");
    const hiHatStandTop = get("Hi-Hat-Stand-Top");
    const hiHatOpenAudio = get("Hi-Hat-Open-Audio");
    const hiHatClosedAudio = get("Hi-Hat-Closed-Audio");

    const floorTomDrumAll = get("Floor-Tom");
    const floorTomAudio = get("Floor-Tom-Audio");

    const snareDrumAll = get("Snare");
    const snareDrum = get("Snare-Drum");
    const snareAudio = get("Snare-Audio");

    const kickDrumAll = get("Kick");
    const kickAudio = get("Kick-Audio");

    // Animations
    const crashTl = gsap.timeline({ paused: true })
      .to(crashCymbol, { rotation: 8, duration: 0.1, transformOrigin: "50% 50%" })
      .to(crashCymbol, { rotation: 0, duration: 1.5, transformOrigin: "50% 50%", ease: Elastic.easeOut.config(2.5, 0.3) });

    const rightTomTl = gsap.timeline({ paused: true })
      .to(rightTomDrum, { scaleX: 1.04, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut })
      .to(rightTomDrum, { scaleY: 0.95, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut }, 0)
      .to(rightTomDrumAll, { rotation: 2.5, duration: 0.1, transformOrigin: "0 50%", ease: Elastic.easeOut }, 0)
      .to(rightTomDrum, { scale: 1, duration: 0.4, transformOrigin: "50% 50%", ease: Elastic.easeOut })
      .to(rightTomDrumAll, { rotation: 0, duration: 0.6, transformOrigin: "0 50%", ease: Elastic.easeOut }, "-=0.4");

    const leftTomTl = gsap.timeline({ paused: true })
      .to(leftTomDrum, { scaleX: 1.04, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut })
      .to(leftTomDrum, { scaleY: 0.95, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut }, 0)
      .to(leftTomDrumAll, { rotation: -2.5, duration: 0.1, transformOrigin: "100% 50%", ease: Elastic.easeOut }, 0)
      .to(leftTomDrum, { scale: 1, duration: 0.4, transformOrigin: "50% 50%", ease: Elastic.easeOut })
      .to(leftTomDrumAll, { rotation: 0, duration: 0.6, transformOrigin: "100% 50%", ease: Elastic.easeOut }, "-=0.4");

    const floorTomTl = gsap.timeline({ paused: true })
      .to(floorTomDrumAll, { scaleX: 1.02, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut })
      .to(floorTomDrumAll, { scaleY: 0.95, duration: 0.1, transformOrigin: "50% 100%", ease: Expo.easeOut }, 0)
      .to(floorTomDrumAll, { scale: 1, duration: 0.4, transformOrigin: "50% 100%", ease: Elastic.easeOut });

    const snareTl = gsap.timeline({ paused: true })
      .to(snareDrum, { scaleX: 1.04, duration: 0.1, transformOrigin: "50% 50%", ease: Expo.easeOut })
      .to(snareDrum, { scaleY: 0.9, duration: 0.1, transformOrigin: "50% 100%", ease: Expo.easeOut }, 0)
      .to(snareDrum, { scale: 1, duration: 0.4, transformOrigin: "50% 100%", ease: Elastic.easeOut });

    const kickTl = gsap.timeline({ paused: true })
      .to(kickDrumAll, { scale: 1.02, duration: 0.1, transformOrigin: "50% 100%", ease: Expo.easeOut })
      .to(kickDrumAll, { scale: 1, duration: 0.4, transformOrigin: "50% 100%", ease: Elastic.easeOut });

    const hiHatUpTl = gsap.timeline({ paused: true })
      .to(hiHatStandTop, { y: "-5%", duration: 0.3, ease: Bounce.easeOut });

    const hiHatTl = gsap.timeline({ paused: true })
      .to([hiHatTop, hiHatBottom], { rotation: -4, duration: 0.1, transformOrigin: "50% 50%" })
      .to([hiHatTop, hiHatBottom], { rotation: 0, duration: 0.6, transformOrigin: "50% 50%", ease: Elastic.easeOut.config(1.5, 0.2) });
    ...
    crashCymbolAll?.addEventListener("click", () => {
      crashTl.restart().play();
      play(crashAudio);
    });

    rightTomDrumAll?.addEventListener("click", () => {
      rightTomTl.restart().play();
      play(smallTomAudio);
    });

    leftTomDrumAll?.addEventListener("click", () => {
      leftTomTl.restart().play();
      play(bigTomAudio);
    });

    floorTomDrumAll?.addEventListener("click", () => {
      floorTomTl.restart().play();
      play(floorTomAudio);
    });

    snareDrumAll?.addEventListener("click", () => {
      snareTl.restart().play();
      play(snareAudio);
    });

    kickDrumAll?.addEventListener("click", () => {
      kickTl.restart().play();
      play(kickAudio);
    });

    hiHatAll?.addEventListener("click", () => {
      hiHati.current++;
      hiHatTl.restart().play();
      if (hiHati.current < 8) {
        hiHatUpTl.reverse();
        play(hiHatClosedAudio);
      } else {
        hiHatUpTl.play();
        play(hiHatOpenAudio);
        hiHati.current = 0;
    ...
    return (
      ...
      `}</style>
            ...
            <audio id="Big-Rack-Tom-Audio">
                <source
                    src="https://s3.amazonaws.com/iamjoshellis-codepen/pens-of-rock/drums/Big-Rack-Tom.mp3"
                    preload="auto"
                    type="audio/mp3"
                />
            </audio>
            <audio id="Crash-Audio">
                <source
                    src="https://s3.amazonaws.com/iamjoshellis-codepen/pens-of-rock/drums/Crash.mp3"
                    preload="auto"
                    type="audio/mp3"
                />
            </audio>
